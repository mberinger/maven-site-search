               ...
               if (set.mode == 'live')
               {
                    for (var i = 0; i < tipuesearch_pages.length; i++)
                    {
                         $.get(tipuesearch_pages[i], '',
                              function (html)
                              {
                                   // MB: not sure how this is supposed to work from the docs
                                   var cont = $(set.liveContent, html).text();
//                                   var cont = $($.parseHTML(html)).filter(set.liveContent).text();
                                   cont = cont.replace(/\s+/g, ' ');
                                   // MB: not sure how this is supposed to work from the docs
                                   var desc = $(set.liveDescription, html).text();
//                                   var desc = $($.parseHTML(html)).filter(set.liveDescription).text();
